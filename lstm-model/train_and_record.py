...
def train_model_for_user(user_id: ObjectId) -> bool:
    try:
        ...
        model = Sequential([
            LSTM(128, activation='relu', return_sequences=True, input_shape=(WINDOW_SIZE, len(active_categories))),
            LSTM(64, activation='relu'),
            Dense(32, activation='relu'),
            Dense(len(active_categories), activation='relu')
        ])

        model.compile(optimizer=Adam(learning_rate=0.001), loss='mse', metrics=[regression_accuracy])

        csv_logger = CSVLogger(f"{MODELS_DIR}/log_{user_id}.csv", append=False)
